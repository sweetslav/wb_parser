...
function fetchAnalytics(dateFrom, dateTo, token) {
  ...
  while (hasMore) {
    ...
    try {
      ...
      if (code !== 200) {
        Logger.log('⚠️ Ошибка Analytics API: ' + response.getContentText());
        break;
      }
  ...
  Logger.log('✅ Analytics: итого получено ' + allProducts.length + ' товаров');
...
function fetchAdvCosts(dateFrom, dateTo, token) {
  ...
  try {
    ...
    if (code !== 200) {
      Logger.log('⚠️ Ошибка /upd: ' + updResponse.getContentText());
      return {};
    }
    ...
    Logger.log('📊 Всего привязок: ' + assignedCount);
...
function fetchAllVendorCodes(token) {
  ...
  try {
    while (hasMore) {
      ...
      Logger.log('📤 Запрашиваем товары (offset ' + offset + ')...');
      
      const response = UrlFetchApp.fetch(url, options);
      const code = response.getResponseCode();
      
      if (code !== 200) {
        Logger.log('⚠️ Ошибка API Цен: ' + response.getContentText());
        break;
      }
      ...
      Logger.log('📦 Загружено ' + allGoods.length + ' товаров');
    ...
    Logger.log('✅ Всего получено товаров: ' + allGoods.length);
